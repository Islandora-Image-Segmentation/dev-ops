...
apt-get -y update
apt-get -y install php7.4-gd php7.4-dev php7.4-xsl php7.4-soap php7.4-curl php7.4-imagick imagemagick lame libimage-exiftool-perl bibutils poppler-utils
cp -v "$SHARED_DIR"/configs/ImageMagick_policy.xml /etc/ImageMagick/policy.xml
chmod 644 /etc/ImageMagick/policy.xml
pecl install uploadprogress
sed -i '/; extension_dir = "ext"/ a\ extension=uploadprogress.so' /etc/php/7.4/apache2/php.ini
...
# wget https://github.com/drush-ops/drush/archive/8.4.5.tar.gz
# tar xf 8.4.5.tar.gz
# sudo mv drush-8.4.5 /opt/
# sudo ln -s /opt/drush-8.4.5/drush /usr/bin/drush
...
# php.ini templating
cp -v "$SHARED_DIR"/configs/php.ini /etc/php/7.4/apache2/php.ini
